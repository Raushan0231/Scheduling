...
const App = () => {
  ...
  const runSchedulingAlgorithm = (data) => {
    ...
    let processList = processes.map((proc) => [
      proc.name,
      parseInt(proc.arrivalTime),
      parseInt(proc.serviceTime),
    ]);
    ...
    let timeline = Array.from({ length: last_instant }, () =>
      new Array(process_count).fill(" ")
    );
    ...
    const roundRobin = (quantum) => {
      ...
      while (queue.length > 0 || arrivalIndex < process_count) {
        while (
          arrivalIndex < process_count &&
          processes[arrivalIndex].arrivalTime <= currentTime
        ) {
        ...
        if (remainingServiceTime[currentProcess] > 0) {
          ...
          turnAroundTime[currentProcess] =
            finishTime[currentProcess] - processes[currentProcess].arrivalTime;
          normTurn[currentProcess] =
            turnAroundTime[currentProcess] /
            processes[currentProcess].serviceTime;
        }
      }
      fillInWaitTime();
    };
      ...
      while (completed < process_count) {
        ...
        for (let i = 0; i < process_count; i++) {
          if (
            ...
            !isCompleted[i] &&
            processes[i].serviceTime < minServiceTime
          ) {
        ...
        turnAroundTime[minIndex] =
          finishTime[minIndex] - processes[minIndex].arrivalTime;
        normTurn[minIndex] =
          turnAroundTime[minIndex] / processes[minIndex].serviceTime;
      ...
      while (completed < process_count) {
        ...
        for (let i = 0; i < process_count; i++) {
          if (
            processes[i].arrivalTime <= currentTime &&
            !isCompleted[i] &&
            remainingServiceTime[i] < minRemainingTime
          ) {
    ...
    const highestResponseRatioNext = () => {
      ...
      while (completed < process_count) {
        ...
        turnAroundTime[maxIndex] =
          finishTime[maxIndex] - processes[maxIndex].arrivalTime;
        normTurn[maxIndex] =
          turnAroundTime[maxIndex] / processes[maxIndex].serviceTime;
      ...
      >
        ...
        >
          ...
          <Button
            ...
            className={`option-button ${
              selectedOption === "Option 4" ? "selected" : ""
            }`}
            style={{ margin: "0 5px" }}
          >
            HRRN
          ...
          <Button
            ...
            className={`option-button ${
              selectedOption === "Option 5" ? "selected" : ""
            }`}
            style={{ margin: "0 5px" }}
          >
            RR
